...
class AutonomousReportGenerator:
    """
    A class responsible for generating comprehensive research reports autonomously.
    It orchestrates the workflow of creating analysts, conducting interviews,
    gathering perspectives, and compiling the final report.
    """

    def __init__(self, llm):
        """
        Initializes the AutonomousReportGenerator with the specified LLM and search engine(Tavily search tool).

        Args:
            llm (LLM): The language model used for generating responses.
            tavily_search (TavilySearchAPIWrapper): The search engine used for web searches.
        """
        self.llm = llm
        self.memory = MemorySaver()
        self.tavily_search = TavilySearchResults(
            tavily_api_key="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"
        )
        self.logger = GLOBAL_LOGGER.bind(module="AutonomousReportGenerator")
        # self.interview_builder = InterviewGraphBuilder(llm, tavily_search)
    # ----------------------------------------------------------------------
    ...
    def write_report(self, state: ResearchGraphState):
        ...
        try:
            if not sections:
                sections = ["No sections generated — please verify interview stage."]
            self.logger.info("Compiling report", topic=topic)
            system_prompt = REPORT_WRITER_INSTRUCTIONS.render(topic=topic)
            report = self.llm.invoke([
                SystemMessage(content=system_prompt),
                HumanMessage(content="\n\n".join(sections))
            ...
            self.logger.error("Error compiling report", error=str(e))
            raise ResearchAnalystException("Failed to compile report", e)
    ...
    def write_introduction(self, state: ResearchGraphState):
        ...
        try:
            ...
            intro = self.llm.invoke([
                SystemMessage(content=system_prompt),
                HumanMessage(content="Generate the introduction.")
            ])
            self.logger.info("Introduction generated successfully", length=len(intro.content))
            return {"introduction": intro.content}
        except Exception as e:
            self.logger.error("Error compiling introduction report", error=str(e))
    ...
    def write_conclusion(self, state: ResearchGraphState):
        ...
        try:
            ...
            conclusion = self.llm.invoke([
                SystemMessage(content=system_prompt),
                HumanMessage(content="Generate the conclusion.")
            ])
            self.logger.info("Conclusion generated successfully", length=len(conclusion.content))
            return {"conclusion": conclusion.content}
        except Exception as e:
            self.logger.error("Error compiling conclusion report", error=str(e))
    ...
    def build_graph(self):
        ...
        try:
            ...
            def initiate_all_interviews(state: ResearchGraphState):
                ...
                return [
                    Send(
                        ...
                        {
                            "analyst": analyst,
                            "messages": [HumanMessage(content=f"So, let us discuss about {topic}.")],
                            "max_num_turns": 2,
                            "context": [],
                            "interview": "",
                            "sections": [],
            ...
            # graph = StateGraph(logger=self.logger)

            # Start and end nodes
            # graph.add_node(START)
            # graph.add_node(END)

            # Add nodes for each step in the report generation process
