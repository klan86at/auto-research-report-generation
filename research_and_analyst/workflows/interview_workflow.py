...
class InterviewGraphBuilder:
    ...
    def __init(self, llm, tavily_search):
        """
        Initializes the InterviewGraphBuilder with the specified LLM and search engine(Tavily search tool).
    ...
    def _search_web(self, state: InterviewState):
        """
        Perform a web search based on the generated question.
        """
        messages = state["messages"]
        question = messages[-1].content

        try:
            self.logger.info("Generating search query from conversation")# question_preview=question[:200])
            structure_llm = self.llm.with_structured_output(SearchQuery)
            search_prompt = GENERATE_SEARCH_QUERY.render()
            search_query = structure_llm.llm.invoke(
                [SystemMessage(content=search_prompt)] + state["messages"]
            )
            # search_query = SearchQuery.parse_raw(search_query_response.content)
            ...
            self.logger.info("Web search completed", num_results=len(search_docs))
            # self.logger.info("Performing web search", query=search_query.query)
            # search_results = self.tavily_search.run(search_query.query)
            # self.logger.info("Web search completed", num_results=len(search_results))

            return {"context": [formatted]}

        except Exception as e:
            self.logger.error("Error during web search", error=str(e))
            raise ResearchAnalystException("Failed to perform web search", e)
    ...
    def _generate_answer(self, state: InterviewState):
        """
        Generate an answer from the expert based on the analyst's question and web search context.
        """
        analyst = state["analyst"]
        messages = state["messages"]
